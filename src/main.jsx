...
function App() {
  ...
  const [reviewLatestImport,setReviewLatestImport] = useState(false);
  const input = useRef();
  ...
  useEffect(() => {
    ...
    apiFetch("/api/imports/latest").then(response=>response.json()).then(data=>setLatestImport(data.latestImport||null)).catch(()=>{});
  }, []);
  ...
  async function handleFiles(files) {
    ...
    try {
      ...
      const { transactions:stored,imported,latestImport:completedImport } = await response.json();
      setTransactions(stored); setFileNames([...new Set(stored.map(row=>row.source).filter(Boolean))]); setView("dashboard");
      setLatestImport(completedImport||null);
      setReviewLatestImport(Boolean(completedImport?.transactionIds?.length));
      setTab("categorise");
      if (imported) {
  ...
  if (view === "dashboard") return <Dashboard {...{...ledgerControls,transactions:filteredTransactions,totalTransactions:transactions.length,updateCategory,updateSubcategory,updateBulkCategory,updateSubscription,updateExcluded,updateAmount,updateReceiptCount,deleteTransaction,deleteTransactions,customCategories,addCategory,renameCategory,deleteCategory,analysis,tab,setTab,fileNames,exportExcel,exportPdf,availableMonths,dateFrom,setDateFrom,dateTo,setDateTo,input,handleFiles,loading,error,notice,latestImport,reviewLatestImport,onReset:()=>{setView("landing");setTransactions([]);setDateFrom("");setDateTo("")}}}/>;
...
function Dashboard({transactions,totalTransactions,updateCategory,updateSubcategory,updateBulkCategory,updateSubscription,updateExcluded,updateAmount,updateReceiptCount,deleteTransaction,deleteTransactions,customCategories,addCategory,renameCategory,deleteCategory,analysis,tab,setTab,fileNames,exportExcel,exportPdf,availableMonths,dateFrom,setDateFrom,dateTo,setDateTo,input,handleFiles,loading,error,notice,latestImport,reviewLatestImport,onReset,ledgers,activeLedgerId,switchLedger,createLedger,renameLedger}) {
...
function Categorise({rows,categories:categoryRecords,latestImport,reviewLatestImport,onAddCategory,onRenameCategory,onDeleteCategory,onCategoryChange,onSubcategoryChange,onBulkCategoryChange,onSubscriptionChange,onExcludedChange,onAmountChange,onReceiptCountChange,onDeleteTransaction,onDeleteTransactions}) {
  ...
  const [latestOnly,setLatestOnly]=useState(Boolean(reviewLatestImport&&latestImport?.transactionIds?.length));
  ...
  const latestIds=new Set(latestImport?.transactionIds||[]);
  useEffect(()=>{if(reviewLatestImport&&latestIds.size)setLatestOnly(true)},[reviewLatestImport,latestImport?.id]);
  ...
  const visible = rows.filter(row => (!latestOnly||latestIds.has(row.id)) && (showCategory==="All" || row.category===showCategory) && matchesSearch(row) && (!subscriptionsOnly||isSubscription(row)) && (visibility==="All" || (visibility==="Excluded"?Boolean(row.isExcluded):!row.isExcluded)));
  useEffect(()=>setPage(0),[search,showCategory,subscriptionsOnly,visibility,latestOnly,rows.length]);
